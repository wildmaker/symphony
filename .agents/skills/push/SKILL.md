...
- Create a PR targeting the ticket base branch if none exists for the branch,
...
2. Determine the base branch from `SYMPHONY_BASE_BRANCH`, falling back to `main`.
3. Run local validation (`make -C elixir all`) before pushing.
...
5. If push is not clean/rejected:
   - If the failure is a non-fast-forward or sync problem, run the `pull`
     skill to merge the ticket base branch, resolve conflicts, and rerun validation.
...
6. Ensure a PR exists for the branch:
   ...
   - If an existing open PR targets the wrong base branch, update it with
     `gh pr edit --base "$base_branch"` when supported, or stop and report the
     mismatch if GitHub refuses the edit.
