...
- `SPEC.md`: intended runtime behavior and workflow contract.
- `docs/customizing-workflow.md`: how to author workflow front matter and prompt bodies.
- `elixir/lib/symphony_elixir/workflow.ex`: workflow loading.
- `elixir/lib/symphony_elixir/config.ex`: runtime config interpretation.
- `elixir/lib/symphony_elixir/orchestrator.ex`: Linear polling and agent lifecycle orchestration.
- `elixir/lib/symphony_elixir/codex/dynamic_tool.ex`: dynamic tool bridge behavior.
...
Purpose:
Poll Linear, create isolated workspaces, launch coding agents, track progress, and shepherd PR-oriented work.

Use this when:

- changing runtime orchestration behavior;
- changing workflow config parsing;
- changing agent process launch or protocol handling;
- changing Linear/GitHub integration behavior.

### Choosing APIs

#### Runtime Configuration

- Use `SymphonyElixir.Config` for runtime config access.
- Avoid ad-hoc environment reads in implementation modules.
- Check `SPEC.md` and config tests for compatibility expectations.

#### Workflow Loading

- Use `SymphonyElixir.Workflow` behavior for workflow front matter and prompt body loading.
- Keep `.agents/WORKFLOW.md` as agent-context infrastructure; do not confuse it with runtime workflow files passed to Symphony.

### Design Explanation

#### Workspace Safety

- Agent workspaces must stay under the configured workspace root.
- Do not run agent turns in the source repo.
- Preserve cleanup and retry semantics when modifying orchestrator behavior.
