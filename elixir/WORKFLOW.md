...
hooks:
  after_create: |
    ...
    git clone --depth 1 --branch "$base_branch" https://github.com/openai/symphony .
...
- The ticket may explicitly set a base branch with a description line like `Base branch: release/1.2` or `基准分支: release/1.2`.
- Symphony exposes that value as `SYMPHONY_BASE_BRANCH` and clones the workspace from it.
- If no ticket base branch is set, use `main`.
- Use the ticket base branch for every base sync, closed-PR restart, conflict merge, and PR target (`gh pr create --base "$base_branch"`).
...
- `pull`: keep branch updated with the ticket base branch before handoff.
...
4. Check whether a PR already exists for the current branch and whether it is closed.
   ...
   - Create a fresh branch from the ticket base branch and restart execution flow as a new attempt.
...
9.  Run the `pull` skill to sync with latest ticket base branch before any code edits, then record the pull/sync result in the workpad `Notes`.
...
9.  Merge latest ticket base branch into branch, resolve conflicts, and rerun checks.
...
5. Create a fresh branch from the ticket base branch.
...
- For closed/merged branch PRs, create a new branch from the ticket base branch and restart from reproduction/planning as if starting fresh.
